...
const HocExercise = () => {
    const SimpleComponent = ({ onLogin, onLogOut, isAuth }) => {
        return (isAuth ? <button className="btn btn-secondary" onClick={onLogOut}>Выйти</button> : <button className="btn btn-primary" onClick={onLogin}>Войти</button>);
    };
    ...
    const withFunctions = (Component) => props => {
        return (
            <CardWrapper>
                <Component isAuth={localStorage.getItem("auth")} onLogin={() => localStorage.setItem("auth", "token")} onLogOut={() => localStorage.removeItem("auth")} />
            </CardWrapper>
        );
    };
